...
export class MemoryAgent {
  ...
  async createNode(nodeData: KnowledgeNodeData): Promise<string> {
    const node = await db.knowledgeNode.create({
      data: {
        ...
        // Native Json — no JSON.stringify needed
        metadata: nodeData.metadata as any,
        tags: (nodeData.tags || []) as any,
      },
    });
  ...
  async reinforceNode(nodeId: string, boost: number = 0.1): Promise<void> {
    ...
    const newNovelty = Math.max(0, node.novelty - 0.1);

    await db.knowledgeNode.update({
      where: { id: nodeId },
      data: { confidence: newConfidence, accessCount: newAccessCount, arousal: newArousal, novelty: newNovelty },
    });

    eventBus.emit({
      type: 'node_update',
      data: { id: nodeId, confidence: newConfidence, accessCount: newAccessCount, arousal: newArousal, novelty: newNovelty },
      timestamp: Date.now(),
    });
  }
  ...
  async createEdge(edgeData: EdgeData): Promise<string> {
    ...
    if (existing) {
      ...
      await db.knowledgeEdge.update({ where: { id: existing.id }, data: { weight: newWeight } });
      eventBus.emit({ type: 'edge_update', data: { id: existing.id, weight: newWeight }, timestamp: Date.now() });
      return existing.id;
    }
    ...
    const edge = await db.knowledgeEdge.create({
      data: {
        ...
        // Native Json — no JSON.stringify needed
        metadata: edgeData.metadata as any,
      },
    });

    eventBus.emit({ type: 'edge_add', data: { id: edge.id, ...edgeData }, timestamp: Date.now() });
    return edge.id;
  }
  ...
  async autoLink(newNodeId: string, similaritySourceId?: string): Promise<void> {
    ...
    // tags is now a native JSON array — no JSON.parse needed
    const newTags: string[] = (newNode.tags as unknown as string[]) ?? [];
    ...
    for (const candidate of candidates) {
      ...
      const candidateTags: string[] = (candidate.tags as unknown as string[]) ?? [];
  ...
  async decayNodes(): Promise<number> {
    ...
    const nodes = await db.knowledgeNode.findMany({ where: { verified: false } });
    ...
    for (const node of nodes) {
      ...
      if (newConfidence < 0.05) {
        ...
        eventBus.emit({ type: 'node_remove', data: { id: node.id, reason: 'decay_below_threshold' }, timestamp: Date.now() });
        decayed++;
      } else {
        await db.knowledgeNode.update({
          where: { id: node.id },
  ...
  async consolidateNodes(): Promise<number> {
    ...
    const toPromote = await db.knowledgeNode.findMany({
      where: { promoted: false, verified: true, confidence: { gte: promoteThreshold } },
    });

    for (const node of toPromote) {
      await db.knowledgeNode.update({ where: { id: node.id }, data: { promoted: true } });
      eventBus.emit({ type: 'node_update', data: { id: node.id, promoted: true, action: 'promoted_to_semantic' }, timestamp: Date.now() });
    }

    const toVerify = await db.knowledgeNode.findMany({
      where: { verified: false, confidence: { gte: verifyThreshold }, accessCount: { gte: 3 } },
    });

    for (const node of toVerify) {
      await db.knowledgeNode.update({ where: { id: node.id }, data: { verified: true } });
      eventBus.emit({ type: 'node_update', data: { id: node.id, verified: true }, timestamp: Date.now() });
    }
  ...
  async query(
    ...
    return {
      nodes: nodes.map(n => ({
        ...
        verified: n.verified,
        promoted: n.promoted,
        // Native Json — no JSON.parse needed
        metadata: (n.metadata as unknown as Record<string, unknown>) ?? {},
        tags: (n.tags as unknown as string[]) ?? [],
      })),
      edges: edges.map(e => ({
        id: e.id,
        sourceId: e.sourceId,
